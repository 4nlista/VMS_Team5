        ...
        if (idStr != null) {
            try {
                ...
                if (deleted) {
                    ...
                    request.setAttribute("message", "Không thể xóa sự kiện (có thể không tồn tại)!");
                }
            } catch (Exception e) {
                e.printStackTrace();
                request.setAttribute("message", "Đã xảy ra lỗi khi xóa sự kiện!");
            }
        }

        //  Quay lại trang danh sách sự kiện
        RequestDispatcher rd = request.getRequestDispatcher("ListEventsServlet");
